## lulik_4.R
## sub ver b) Liquid steady state model generalizable to N2O
## BUG:
##     model.matrix with formula interface changed data, wired!
...
lulik <- function(
  ...
  p, RQ,  Depth_O2, Depth_CH4,Depth,   ###CORRECT DEPTH and TOTAL MICROSITE HERE
  ...
  ))
{
  ...
  EaVmaxSx <- z[3]
  KMSx <- z[4]       
  Depth <- z[5] ###ADD DEPTH and TOTAL MICROSITE HERE
  total.microsite <- z[6]
  ...
  l.mat0 <- as.matrix(data[,all.vars(formula)]) #Need to O2 in the formula for PR
  ...
  SoilM__ <- l.mat[,1] 
  SoilT__ <- l.mat[,2] #Need to O2 for PR
  Rh__ <- l.mat[,3]
  CH4__ <- l.mat[,4]
  ...
  #for(i in 1:n){  
    ...
    tmp <- damm(
      ...
      Depth_O2=Depth_O2, Depth_CH4=Depth_CH4, Depth=Depth,###CORRECT DEPTH and TOTAL MICROSITE HERE
      microsite = microsite, total.microsite=total.microsite,
      time.step = time.step, 
      iO2 = iO2, iCH4 = iCH4,
      flagO2 = flagO2, flagCH4 = flagCH4,
      KmO2_Rh = KmO2_Rh, alphaVmaxSx = alphaVmaxSx,   EaVmaxSx = EaVmaxSx, ###ADD DEPTH and TOTAL MICROSITE HERE
  ...
  ## return
  list(data=cbind(SoilM__,SoilT__,Rh__,CH4__),
       fit_Rh=meanRH,fit_CH4=meanCH4,par=x,tpar=z,
       flag=do.call(cbind,flag),
       output=do.call(rbind,olst),
       O2.lst=O2.lst,
       CH4.lst=CH4.lst,elapsed=proc.time()-ts0)
}
...
test <- function() #Error in if (log) { : argument is of length zero
{
  ...
  debug(lulik)
  r0 <- lulik(
    x=x0,formula=~SoilM+SoilT+Rh+CH4,data=trench,nlevels=100,
    R=0.008314472, O2airfrac=0.209,  CH4airfrac = 1.8,BD=1.009, PD=2.52,Porosity=(1-0.13)*0.6,
    p=0.00046,RQ=1, Depth_O2 = 10, Depth_CH4 = 10, microsite = 0,total.microsite = 10000, time.step = 300,
    hyper=hyper
  )
